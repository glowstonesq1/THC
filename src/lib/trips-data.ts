export type TripStatus = 'upcoming' | 'past' | 'full' | 'open';
...
export interface Trip {
  ...
  spotsRequired: number; // minimum needed to run
  price: string;
  status: TripStatus;
  highlights: string[];
  category: 'young-wild' | 'old-wise' | 'all';
  image?: string;
  pastGallery?: string[];
}

const TRIPS_FILE_KEY = 'thc_trips_data';
...
export function getTrips(): Trip[] {
  ...
  try {
    const stored = localStorage.getItem(TRIPS_FILE_KEY);
...
export function saveTrips(trips: Trip[]): void {
  ...
  localStorage.setItem(TRIPS_FILE_KEY, JSON.stringify(trips));
}

export function updateTripSpots(id: string, spotsAvailable: number, spotsRequired: number): Trip[] {
  const trips = getTrips();
  const updated = trips.map(t =>
    t.id === id ? { ...t, spotsAvailable, spotsRequired } : t
  );
  saveTrips(updated);
  return updated;
}
